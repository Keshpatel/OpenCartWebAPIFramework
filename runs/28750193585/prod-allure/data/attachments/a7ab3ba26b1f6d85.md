...
- Name: homepagefix.spec.ts >> @sanity Home Page title test with fixtures
- Location: tests/homepagefix.spec.ts:17:1
        ...
        - generic [ref=e87]:
          ...
          - generic [ref=e91]:
            - generic [ref=e92]:
              ...
              - textbox "E-Mail Address" [ref=e94]: tom_smith@3dTechnode.ca
            - generic [ref=e95]:
              - generic [ref=e96]: Password
              - textbox "Password" [ref=e97]: "!Spring123"
...
```ts
  ...
  5  |     await loginPage.doLogin(process.env.APP_USERNAME!, process.env.APP_PASSWORD!);
  6  | });
  7  | 
  8  | //common test for all pages 
  9  | test('verify logo and footer on home page test', async({basePage}) => {
  10 |     expect(await basePage.isLogoVisible()).toBeTruthy();
  11 | });
  12 | 
  13 | test('verify all footers exist on the page test', async({basePage}) => {
  14 | expect(await basePage.checkFootersCounts()).toBe(16);
  15 | });
  16 | 
  17 | test('@sanity Home Page title test with fixtures', async({homePage}) => {
  18 |     const pageTitle = await homePage.getPageTitle();
  19 |     console.log('Home Page title : ' , pageTitle);
> 20 |     expect(pageTitle).toBe('My Account');
     |                       ^ Error: expect(received).toBe(expected) // Object.is equality
  21 | });
  22 | 
  23 | test('log out link exist test with fixtures', async({homePage}) => {
  24 |     expect(await homePage.isLogoutLinkExist()).toBeTruthy();
  25 | });
  26 | 
  27 | test('@regression  Home Page Headers test with fixtures', async({homePage}) => {
  28 |     let allHeaders = await homePage.getHomePageHeaders();
  29 |     console.log('home page headers: ', allHeaders);
  30 |     expect.soft(allHeaders).toHaveLength(4);
  31 |     expect.soft(allHeaders).toEqual([
  32 |         'My Account', 'My Orders', 'My Affiliate Account', 'Newsletter'
  33 |     ])
  34 | });
  35 | 
  36 | 
